...
pub fn build(b: *std.Build) !void {
    ...
    if (target.query.os_tag == .emscripten) {
        ...
        const run_info = std.fmt.allocPrint(std.heap.page_allocator, "Run {s}", .{project_name}) catch "format failed";
        const run_option = b.step("run", run_info);
    ...
    const run_step = b.step("run", "Run projcec");
